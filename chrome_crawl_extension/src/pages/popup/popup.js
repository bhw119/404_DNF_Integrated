  ...
  try{
    ...
    if(chrome?.sidePanel?.setOptions){
      await chrome.sidePanel.setOptions({
        tabId: activeTab.id,
        path: `src/pages/sidepanel_analysis/index.html?doc=${encodeURIComponent(savedDocId)}`
      });
    }
  ...
  try{
    ...
    try{
      ...
      if(chrome?.sidePanel?.setOptions && chrome?.sidePanel?.open){
        ...
        await chrome.sidePanel.setOptions({
          tabId: activeTab.id,
          path: savedId ? `src/pages/sidepanel_summary/index.html?doc=${encodeURIComponent(savedId)}` : `src/pages/sidepanel_summary/index.html`
        });
        await chrome.sidePanel.open({ tabId: activeTab.id });
      }
